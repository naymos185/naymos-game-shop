...
import { Gamepad2, ChevronRight, Layers } from 'lucide-react';
...
export function GameCategorySection({
  ...
  // Extract unique categories safely, excluding duplicates and any category named 'ทั้งหมด'
  const categories = useMemo(() => {
    const set = new Set<string>();
    games.forEach((g) => {
      const raw = (g.category || '').trim();
      if (raw && raw !== 'ทั้งหมด' && raw.toLowerCase() !== 'all') {
        set.add(raw);
      }
    });
  ...
  const filteredGames = useMemo(() => {
    ...
    return games.filter((g) => {
      ...
      // If game is ROV and its DB category is 'ทั้งหมด', also match if selected is MOBA
      if (g.slug === 'rov' && cat === 'ทั้งหมด' && selectedCategory === 'MOBA') {
        return true;
      }
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8">
      ...
      {/* Category Pills Navigation (No emojis, distinct active state, horizontally scrollable) */}
        ...
        {categories.map((cat) => {
          ...
          const count =
            cat === 'ทั้งหมด'
              ...
              : games.filter((g) => {
                  ...
                  if (g.slug === 'rov' && c === 'ทั้งหมด' && cat === 'MOBA') return true;
                  return c === cat;
                }).length;
          ...
          return (
            <button
              ...
              className={`shrink-0 inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs sm:text-sm font-bold transition-all duration-150 cursor-pointer select-none active:scale-95 ${
                isActive
      ...
      {/* Category Container Background (Visual container framing the active category) */}
      <div className="rounded-3xl border border-sky-100/90 bg-gradient-to-b from-sky-50/70 via-sky-50/30 to-white p-3.5 sm:p-5 shadow-xs transition-all duration-200">
        <div className="flex items-center justify-between mb-3 px-1">
          <div className="flex items-center gap-1.5 text-xs font-bold text-sky-800">
            <Layers className="w-3.5 h-3.5 text-sky-500" />
            <span>
              {selectedCategory === 'ทั้งหมด'
                ? `เกมทั้งหมด (${filteredGames.length})`
                : `หมวดหมู่: ${selectedCategory} (${filteredGames.length})`}
            </span>
          </div>
          {selectedCategory !== 'ทั้งหมด' && (
            <button
              ...
              className="text-[11px] font-semibold text-sky-600 hover:text-sky-700 underline underline-offset-2 cursor-pointer"
            >
              แสดงทั้งหมด
            </button>
          )}
        ...
        >
          {filteredGames.length > 0 ? (
            filteredGames.map((game, index) => {
              // Normalize badge text so ROV doesn't show "ทั้งหมด"
              const badgeCategory =
                game.slug === 'rov' && (game.category === 'ทั้งหมด' || !game.category)
                  ? 'MOBA'
                  : game.category && game.category !== 'ทั้งหมด'
                  ? game.category
                  : 'Game';
            ...
            <div className="col-span-full py-12 text-center bg-white rounded-2xl border border-sky-100 p-8">
              <p className="font-bold text-slate-700 text-sm">ไม่พบเกมในหมวดหมู่นี้</p>
              <button
                type="button"
                onClick={() => setSelectedCategory('ทั้งหมด')}
                className="mt-3 inline-flex items-center gap-1 px-4 py-1.5 rounded-full bg-sky-100 text-sky-700 text-xs font-bold hover:bg-sky-200 active:scale-95 transition-all duration-150 cursor-pointer"
              >
                ดูเกมทั้งหมด
              </button>
            </div>
