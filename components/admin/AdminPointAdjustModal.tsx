  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 px-2.5 py-1 text-xs font-medium text-zinc-200 transition"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl space-y-4">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">{user.full_name || user.email} (ปัจจุบัน: {user.balance} พ้อย)</p>
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-1.5 rounded-xl text-xs font-semibold transition ${
                  mode === 'deduct' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-white'
                }`}
              >
                หักคะแนน
              </button>
              <button
                ...
                onClick={() => setMode('add')}
                className={`flex-1 py-1.5 rounded-xl text-xs font-semibold transition ${
                  mode === 'add' ? 'bg-emerald-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-white'
                }`}
              >
                เพิ่มคะแนน
              </button>
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">จำนวนคะแนน</label>
              <input
                type="number"
                value={points}
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">เหตุผล / หมายเหตุ</label>
              <input
                type="text"
                value={reason}
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 rounded-xl border border-zinc-700 py-2 text-xs text-zinc-300 hover:bg-zinc-800"
              >
                ยกเลิก
              </button>
              <button
                ...
                className={`flex-1 rounded-xl py-2 text-xs font-bold text-white flex items-center justify-center gap-1.5 ${
                  mode === 'deduct' ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'
                }`}
              >
